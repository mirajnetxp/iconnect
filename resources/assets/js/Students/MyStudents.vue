    ...
    <div class="container">
            ...
            <div class="row">
                <template>
                    <div v-if="auth.user_role_id == 2" class="form-group col-md-2 text-center">
                        <select name="school level" @change="getFilterSchools" id="school_level" v-model="filter.level"
                                class="form-control">
                    ...
                    <div v-if="auth.user_role_id == 2" class="form-group col-md-2 text-center">
                        <select name="school name" @change="getFilterMentors" id="school_name" class="form-control"
                                v-model="filter.schoolId">
                            <option v-for="school in schools" :value="school.id" :key="school.id">{{school.name}}
                            </option>
                            <option value="0">School Name(No Selected)</option>
                        </select>
                    </div>
                    <div v-if="auth.user_role_id == 2 || auth.user_role_id == 3"
                         class="form-group col-md-2 text-center">
                        <select name="mentor" id="mentor" @change="reloadPage" class="form-control"
                    ...
                    <div class="form-group col-md-2 text-center">
                        <input type="text" v-model="filter.searchKeyword" @change="reloadPage"
            ...
            <table class="table table-hover my-students">
                ...
                <tr v-for="(student, key) in students.data" :key="student.id">
                    ...
                    <td class="actions text-center">
                        <a v-if="auth.user_role_id != 4" href="#" class="btn btn-large btn-cta"
                           @click.prevent="newStudent(student.id)">Edit</a>
        ...
        <transfer-modal v-if="transferModal" @close="transferModal = false" :student-info="selected_student"
            ...
            <h1 slot="header" class="text-center">
                Transfer <span> {{selected_student.first_name}} {{selected_student.last_name}} </span> To</h1>
        </transfer-modal>
    </div>
</template>
